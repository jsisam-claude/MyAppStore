...
import app.grapheneos.apps.util.RepoAuth
...
// Glide does not use openConnection(), so the repository access key has to be
// attached here as well. GlideUrl keys its cache on the URL only, so adding
// headers does not change caching behaviour.
private fun iconModel(url: String): GlideUrl {
    val headers = RepoAuth.headers(url)
    if (headers.isEmpty()) {
        return GlideUrl(url)
    }
